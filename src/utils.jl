...
function load_pq_curves(
    ...
    for max_gen in range_quota
        results_df[max_gen] = Dict("DA" => SimulationResults(joinpath(simulation_folder, "da_quota_$max_gen", "1")),
        "RT" => SimulationResults(joinpath(simulation_folder, "rt_quota_$max_gen", "1"))) 
...
function load_pq_curves(
    ...
    results_df = Dict()
    for max_gen in range_quota
        results_df[max_gen] = Dict("RT" => SimulationResults(joinpath(simulation_folder, "rt_quota_$max_gen", "1"))) 
        lmps_df[max_gen] = evaluate_prices(market_simulator, results_df[max_gen])
    end
    return lmps_df, results_df
end
...
"""
    plot_revenue_curves_renewable(
        market_simulator::UCEDRT, lmps_df::Dict{Any, Any}, results_df::Dict{Any, Any}, bids::Vector{Float64},generator_name::String,
        node::String,
    )
...
function plot_generation_curves_renewable(
    ...
    for (j,q) in enumerate(bids) 
        ...
        for i=1:(round(Int,length(gen_rt_aux)/12)) #TODO: Change 12 to horizon
            gen_rt = vcat(gen_rt,[sum(gen_rt_aux[(1+12*(i-1)):(12+12*(i-1))])/12])
        end
